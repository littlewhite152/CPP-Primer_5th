...
​	**因为使用重载的运算符本质上是一次函数调用，所以这些关于运算对象求值顺序的规则无法应用到重载的运算符上，特别是逻辑与运算符、逻辑或运算符和逗号运算符**

​	**不重载逗号运算符和取地址运算符的原因：C++语言已经定义了这两种运算符用于类类型对象时的特殊含义，这一点与大多数运算符都不相同。因为这两种运算符已经有了内置的含义，所以一般来说他们不应该被重载**
...
通常情况下，输出运算符的第一个形参是`ostream`类型的普通引用（非常量是因为向流写入内容会改变状态，引用是因为无法直接赋值一个ostream对象），第二个形参是要打印类型的常量引用，返回值是它的`ostream`形参。
