...
因为当参数是对象时，调用参数时实参与形参进行值传递，此时的形参没有定义所以也会调用拷贝构造函数，然后在调用拷贝构造函数的时候又会出现同样的问题，如此一来反复调用，变成了死循环

**为什么参数类型要是const？**

1. 避免用户将临时对象绑定到非const引用上
...
- （=）用`=`定义变量。
- （参数传递）将对象作为实参传递给非引用类型的形参。
- （函数返回）从返回类型为非引用类型的函数返回对象。
- （花括号列表初始化）用花括号列表初始化数组中的元素或聚合类中的成员。
...
局部作用域定义的对象是在栈分配的，不用手动释放。而指针指向的是堆分配的内存，需要手动释放。

### 三/五法则（The Rule of Three/Five）

**需要析构函数的类一般也需要拷贝和赋值操作。**因为不这样的话，拷贝的对象和被拷贝的对象知识简单的进行值传递，他们指向的内存还是同一个，所以在离开作用域或者直接销毁的时候会调用析构函数两次，即同一块内存被释放两次

需要拷贝构造的类一般也需要拷贝赋值操作，反之亦然。
...
普通的拷贝构造函数和类值得拷贝构造函数的区别：前者运算符左右两边的对象指向的是一个内存，后者则不是
